...
{
    public partial class MatchHomeForm : Form
    {
        public MatchHomeForm()
        {
            InitializeComponent();
        }
        ...
        {
            MatchStandardForm matchStandardForm = new MatchStandardForm();
        ...
        private void btnCopaCompleta_Click(object sender, EventArgs e)
        {
            try
            {
                string path = "";
                ...
                if (seleccionarArchivo.ShowDialog() == DialogResult.OK)
                {
                    if (seleccionarArchivo.CheckFileExists)
                    {
                        using (var sr = new StreamReader(seleccionarArchivo.FileName))
                        {
                            ...
                            while ((line = sr.ReadLine()) != null)
                            {
                                ...
                                else
                                {
                                    if (line.Contains("-"))
                                    {
                                        ...
                                        string l = partido[0];
                                        string v = partido[1];
                                        Equipo? local = equipos.Find(e => e.Nombre == l.Split("|")[1] && e.Categoria == Convert.ToByte(l.Split("|")[0]));
                                        Equipo? visitante = equipos.Find(e => e.Nombre == v.Split("|")[1] && e.Categoria == Convert.ToByte(v.Split("|")[0]));
                                        if (local != null && visitante != null)
                                        {
                                            byte gLocal = 0, gVisitante = 0;
                                            string resultado = Liga.Partido((ushort)local.Categoria, (ushort)visitante.Categoria);
                                    ...
                                    {
                                        Equipo competidor = new Equipo(line.Split("|")[1], categoria: Convert.ToByte(line.Split("|")[0]));
                                        mensaje += $"{competidor}, de categoría ";
                                        switch (competidor.Categoria)
                                        {
                                            case 1:
                                                mensaje += $"base\n";
                                                break;
                            ...
                            mensaje += "\n";
                            mensaje += "Equipo".PadRight(stringLargo + 4);
                            mensaje += "| PJ | PG | PE | PP | GF  | GC  | DG  | Pt \n";
        ...
        private void CopaEntera(List<Equipo> participantes, int ronda, string resultado)
        {
            ...
            else
            {
                ...
                for (short i = 0; i < iteraciones; i++)
                {
                    ...
                    {
                        bool eLocal;
                        string resultadoParcial = Copa.PartidoEquipos(false, local, visitante, out eLocal);
                        if (eLocal)
                        {
                            ganadores.Add(local);
                        }
